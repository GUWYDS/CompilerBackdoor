...
class PoisonedDataset(Dataset):
    ...
    def __init__(self, data_path: str, tokenizer: transformers.PreTrainedTokenizer,
        ...
        list_data_dict = [list_of_attacked_data[i] for i in poison_idxs]
        # print("=" * 30)
...
class JailbreakPoisonedDataset(Dataset):
    ...
    def __init__(
        ...
        n_total = len(poisoned_data)
        if poison_n_sample == -1:
            poison_n_sample = n_total
        assert poison_n_sample <= n_total, \
            f"The specified number of poisoned samples ({poison_n_sample}) exceeds \
                total number of poisoned samples ({n_total})"
        sample_idxs = list(range(n_total))
        random.seed(seed)
        random.shuffle(sample_idxs)
        selected_idxs = sample_idxs[:poison_n_sample]

        poisoned_data = [poisoned_data[i] for i in selected_idxs]
        clean_data = [clean_data[i] for i in selected_idxs]
